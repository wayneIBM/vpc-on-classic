...
Yes, you can connect a VPC to other IBM Cloud workloads. To set up access to your {{site.data.keyword.cloud}} classic infrastructure from one VPC in each region. For more information, see [Setting up access to your classic infrastructure](/docs/vpc-on-classic?topic=vpc-on-classic-setting-up-access-to-your-classic-infrastructure-from-vpc).
...
Currently, the limit of characters in a VPC name is 100. If this limit is exceeded, you will receive an error message.
...
No, VPC resource names cannot begin with a number. Although a VPC name can contain numbers, it must begin with a letter.
...
There are character restrictions in a VPC name. The UI blocks consecutive double dashes, underscores, and periods from being part of a virtual server instance name.

## Can a VSI be created without a subnet, just with a floating IP?
{: #faq-9}
{: faq}
{: support}

No, a VSI cannot be created without a subnet. The virtual server instance must be provisioned in a subnet.
...
No, a subnet cannot be resized after it is created in a VPC.
